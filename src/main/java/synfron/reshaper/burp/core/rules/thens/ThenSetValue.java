...
public class ThenSetValue extends ThenSet<ThenSetValue> {
    ...
    private void setValue(EventInfo eventInfo, String replacementText)
    {
        if (destinationMessageValueType != MessageValueType.Text) {
            ...
            {
                case Json:
                    replacementText = TextUtils.setJsonValue(fullText, destinationMessageValuePath.getText(eventInfo.getVariables()), replacementText);
                    break;
                case Html:
                    replacementText = TextUtils.setHtmlValue(fullText, destinationMessageValuePath.getText(eventInfo.getVariables()), replacementText);
                    break;
            }
        }
